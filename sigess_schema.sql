...
BEGIN
  -- Gerar grupo_id único para todos os DAEs agrupados/anuais desta sessão (antes do loop)
  ...
  LOOP
    INSERT INTO public.financeiro_dae (
      ...
      p_data_pagamento,
      -- Atribui o v_grupo_id gerado se não for unitario
...
-- ATUALIZADO: usa `used` em vez de `foto_base64 IS NULL` para controle de uso único
...
DECLARE
  ...
  v_item          jsonb;
BEGIN
  -- 1. Buscar metadados comuns a partir de QUALQUER membro do grupo (incluindo cancelados)
  ...
  -- 2. Cancelar todos os membros do grupo original
  ...
  -- 3. Re-inserir cada item do grupo com seus novos valores individuais
...
-- ATUALIZADO: adicionado `foto_url`, `used` e removido `updated_at` (alinhado ao banco real)
...
CREATE TABLE public.parametros_financeiros (
    id uuid DEFAULT gen_random_uuid() NOT NULL PRIMARY KEY,
    regime_padrao text NOT NULL DEFAULT 'anuidade'::text CHECK (regime_padrao = ANY (ARRAY['anuidade'::text, 'mensalidade'::text])),
    dia_vencimento integer NOT NULL DEFAULT 1 CHECK (dia_vencimento >= 1 AND dia_vencimento <= 28),
    ano_base_cobranca integer NOT NULL DEFAULT 2024,
    ...
    anos_atraso_alerta integer NOT NULL DEFAULT 1,
...
CREATE TABLE public.tipos_cobranca (
    id uuid DEFAULT gen_random_uuid() NOT NULL PRIMARY KEY,
    categoria text NOT NULL CHECK (categoria = ANY (ARRAY['contribuicao'::text, 'cadastro_governamental'::text])),
    nome text NOT NULL,
    descricao text,
...
-- ATUALIZADO: colunas obrigatórias alinhadas ao banco (NOT NULL real)
CREATE TABLE public.financeiro_lancamentos (
    id uuid DEFAULT gen_random_uuid() NOT NULL PRIMARY KEY,
    socio_cpf text NOT NULL REFERENCES public.socios(cpf),
    sessao_id uuid NOT NULL DEFAULT gen_random_uuid(),
    tipo text NOT NULL CHECK (tipo = ANY (ARRAY['anuidade'::text, 'mensalidade'::text, 'inicial'::text, 'transferencia'::text, 'contribuicao'::text, 'cadastro_governamental'::text])),
    ...
    forma_pagamento text NOT NULL CHECK (forma_pagamento = ANY (ARRAY['dinheiro'::text, 'pix'::text, 'transferencia'::text, 'boleto'::text, 'cartao'::text])),
    descricao text,
    status text NOT NULL DEFAULT 'pago'::text CHECK (status = ANY (ARRAY['pago'::text, 'cancelado'::text])),
...
CREATE TABLE public.financeiro_cobrancas_geradas (
    id uuid DEFAULT gen_random_uuid() NOT NULL PRIMARY KEY,
    tipo_cobranca_id uuid NOT NULL REFERENCES public.tipos_cobranca(id),
    socio_cpf text NOT NULL REFERENCES public.socios(cpf),
    valor numeric NOT NULL,
    data_lancamento date NOT NULL DEFAULT CURRENT_DATE,
    data_vencimento date,
    lancamento_id uuid REFERENCES public.financeiro_lancamentos(id),
    status text NOT NULL DEFAULT 'pendente'::text CHECK (status = ANY (ARRAY['pendente'::text, 'pago'::text, 'cancelado'::text])),
    cancelado_em timestamp with time zone,
    cancelado_por uuid REFERENCES public."User"(id),
    cancelamento_obs text,
    created_at timestamp with time zone DEFAULT now(),
...
CREATE TABLE public.financeiro_dae (
    id uuid DEFAULT gen_random_uuid() NOT NULL PRIMARY KEY,
    socio_cpf text NOT NULL REFERENCES public.socios(cpf),
    tipo_boleto text NOT NULL CHECK (tipo_boleto = ANY (ARRAY['unitario'::text, 'agrupado'::text, 'anual'::text])),
...
CREATE TABLE public.financeiro_historico_regime (
    id uuid DEFAULT gen_random_uuid() NOT NULL PRIMARY KEY,
    socio_cpf text NOT NULL REFERENCES public.socios(cpf),
    regime text NOT NULL CHECK (regime = ANY (ARRAY['anuidade'::text, 'mensalidade'::text])),
