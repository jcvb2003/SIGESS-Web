      ...
      <div className="rounded-2xl border border-emerald-400/15 bg-[rgba(10,34,20,0.75)] backdrop-blur-xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] p-6 sm:p-8 space-y-6">
        ...
        <FormProvider {...methods}>
          <form onSubmit={methods.handleSubmit(onSubmit)} className="space-y-4">
                      ...
                      >
                        <Fish
                          className={`h-5 w-5 transition-transform duration-300 ${
                            showPassword ? "scale-x-[-1] text-emerald-400" : ""
                          }`}
                        />
                      </button>
                    </div>
            ...
            <Button
              className="w-full h-12 bg-emerald-600 hover:bg-emerald-500 text-white font-semibold rounded-xl shadow-lg shadow-emerald-900/30 transition-all duration-200 hover:shadow-emerald-800/40 mt-2"
              type="submit"
              disabled={loading}
            >
              {loading ? (
