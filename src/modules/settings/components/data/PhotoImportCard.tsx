...
export function PhotoImportCard() {
  ...
  const { isImporting, progress, currentFile, report, importPhotos, cancelImport } = usePhotoImport()
  ...
  const handleSelectFolder = async () => {
    try {
      ...
      if (error.name !== 'AbortError') {
        ...
        toast.error("Erro ao selecionar pasta. Certifique-se de usar um navegador compatível (Chrome/Edge).")
      }
    }
  }
  ...
  const handleOpenChange = (newOpen: boolean) => {
    if (!newOpen && isImporting) {
      toast.warning("Aguarde o término da importação ou cancele-a antes de fechar.")
      return
    }
    setOpen(newOpen)
  }
  ...
  return (
    <Card className="border-border/50 shadow-sm h-full">
      <CardHeader>
        ...
        <CardDescription>
          Utilize o navegador para importar fotos dos sócios direto de uma pasta.
        </CardDescription>
      </CardHeader>
      <CardContent className="flex flex-col gap-3 border-t border-border/10 pt-4">
        <div className="flex items-center gap-3 rounded-md border border-dashed border-border/60 bg-muted/30 px-3 py-2">
          ...
          <p className="text-xs text-muted-foreground">
            Suporte focado em navegadores Chromium (Chrome, Edge, Opera). No Firefox a funcionalidade será limitada.
          </p>
        </div>
        ...
        <Dialog open={open} onOpenChange={handleOpenChange}>
          ...
          <DialogContent className="sm:max-w-lg">
            <DialogHeader>
              ...
              <DialogDescription>
                Importe fotos dos sócios automaticamente. O nome do arquivo deve corresponder ao CPF do sócio (apenas números).
              </DialogDescription>
            </DialogHeader>
              ...
              {!isImporting && !report && (
                <div className="space-y-4">
                  <div className="space-y-3">
                    ...
                    <RadioGroup value={mode} onValueChange={(v) => setMode(v as 'all' | 'newOnly')}>
                      <div className="flex items-center space-x-2 border rounded-md p-3 hover:bg-muted/50 transition-colors">
                        ...
                        <Label htmlFor="r1" className="cursor-pointer flex-1">
                          <span className="font-medium block">Importar todas</span>
                      ...
                      <div className="flex items-center space-x-2 border rounded-md p-3 hover:bg-muted/50 transition-colors">
                        ...
                        <Label htmlFor="r2" className="cursor-pointer flex-1">
                          <span className="font-medium block">Apenas novas</span>
                          <span className="text-xs text-muted-foreground font-normal">
                  ...
                  <div className="bg-blue-500/10 border border-blue-500/20 rounded-md p-3 flex gap-3 items-start">
                    ...
                    <div className="text-sm text-blue-700 dark:text-blue-400">
                      ...
                      <ul className="list-disc list-inside space-y-1 text-xs">
                        ...
                        <li>Nome do arquivo deve ser o CPF (ex: 12345678900.jpg)</li>
                        <li>Use navegadores baseados em Chromium (Chrome, Edge)</li>
                      </ul>
                    </div>
                  </div>

                  <Button onClick={handleSelectFolder} className="w-full h-11 text-base">
              ...
              {isImporting && (
                <div className="space-y-6">
                  <div className="space-y-2 text-center">
                    ...
                    <p className="text-sm text-muted-foreground">Processando: {currentFile}</p>
                  </div>
                  ...
                  <Button variant="destructive" onClick={cancelImport} className="w-full">
              ...
              {report && !isImporting && (
                <div className="space-y-4">
                  <div className="text-center space-y-2">
                    ...
                    <h3 className="text-lg font-medium">Importação Finalizada</h3>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="bg-green-50 dark:bg-green-900/20 p-3 rounded-md text-center border border-green-100 dark:border-green-900/30">
                      <span className="block text-2xl font-bold text-green-600 dark:text-green-400">{report.success}</span>
                      <span className="text-xs text-green-700 dark:text-green-300">Sucesso</span>
                    </div>
                    <div className="bg-red-50 dark:bg-red-900/20 p-3 rounded-md text-center border border-red-100 dark:border-red-900/30">
                      <span className="block text-2xl font-bold text-red-600 dark:text-red-400">{report.failed}</span>
                      <span className="text-xs text-red-700 dark:text-red-300">Falhas</span>
                    </div>
                    <div className="bg-yellow-50 dark:bg-yellow-900/20 p-3 rounded-md text-center border border-yellow-100 dark:border-yellow-900/30">
                      <span className="block text-2xl font-bold text-yellow-600 dark:text-yellow-400">{report.skipped}</span>
                      <span className="text-xs text-yellow-700 dark:text-yellow-300">Pulados</span>
                    </div>
                    <div className="bg-gray-50 dark:bg-gray-900/20 p-3 rounded-md text-center border border-gray-100 dark:border-gray-900/30">
                      <span className="block text-2xl font-bold text-gray-600 dark:text-gray-400">{report.notFound}</span>
                      <span className="text-xs text-gray-700 dark:text-gray-300">Não Encontrados</span>
                    </div>
                  </div>

                  {report.details.length > 0 && (
                    <div className="space-y-2">
                      <Label className="text-sm font-medium">Detalhes de Erros/Falhas</Label>
