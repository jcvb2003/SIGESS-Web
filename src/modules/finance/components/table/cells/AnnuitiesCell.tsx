import type { MemberFinancialSummary } from "../../../types/finance.types";
import { getRequiredYears } from "../../../domain/annuityRules";
import { cn } from "@/shared/lib/utils";
import {
  Tooltip,
  TooltipContent,
  TooltipProvider,
  TooltipTrigger,
} from "@/shared/components/ui/tooltip";

interface AnnuitiesCellProps {
  readonly member: MemberFinancialSummary;
  readonly currentYear: number;
  readonly anoBase: number;
}

export function AnnuitiesCell({ member, currentYear, anoBase }: AnnuitiesCellProps) {
  if (member.isento) {
    return (
      <span className="inline-flex items-center rounded-full bg-blue-50 dark:bg-blue-900/30 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-blue-700 dark:text-blue-400 border border-blue-100 dark:border-blue-800">
        Isento
      </span>
    );
  }

  if (member.regime === "mensalidade") {
    const paidMonths = member.mesesPagosAtual ?? [];
    const dueMonthCount = new Date().getMonth() + 1;
    const paidCurrentCount = paidMonths.filter((month) => month >= 1 && month <= dueMonthCount).length;
    const paidYearCount = paidMonths.filter((month) => month >= 1 && month <= 12).length;
    const isFullySettled = paidYearCount > dueMonthCount;

    return (
      <div className="leading-tight">
        <div className="text-xs font-semibold text-foreground">
          {paidYearCount}/{dueMonthCount} meses
        </div>
        <div className="flex items-center gap-1 text-[10px] font-medium text-muted-foreground">
          <span>{currentYear}</span>
          {isFullySettled ? (
            <span className="rounded-full bg-success/10 px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wide text-success">
              Quitado
            </span>
          ) : paidCurrentCount >= dueMonthCount ? (
            <span className="inline-flex h-2 w-2 rounded-full bg-success" aria-hidden="true" />
          ) : null}
        </div>
      </div>
    );
  }

  const paid = member.anuidadesPagas ?? [];
  const years = getRequiredYears(anoBase, currentYear).map((y) => ({
    year: y,
    isPaid: paid.includes(y),
  }));

  // Show only the last 3 years for compact display
  const displayYears = years.slice(-3);

  return (
    <div className="flex items-center gap-1.5">
      <TooltipProvider>
        {displayYears.map((y) => (
          <Tooltip key={y.year}>
            <TooltipTrigger asChild>
              <div
                className={cn(
                  "flex h-6 w-6 items-center justify-center rounded-full text-[10px] font-bold border transition-all cursor-default shadow-sm",
                  y.isPaid
                    ? "bg-success/20 border-success/30 text-success font-bold shadow-sm"
                    : "bg-red-50 dark:bg-red-900/30 border-red-100 dark:border-red-800 text-red-600 dark:text-red-400 opacity-60 hover:opacity-100"
                )}
              >
                {String(y.year).slice(-2)}
              </div>
            </TooltipTrigger>
            <TooltipContent side="top" className="text-[10px] px-2 py-1">
              Anuidade {y.year}: {y.isPaid ? "Paga ✓" : "Pendente ✗"}
            </TooltipContent>
          </Tooltip>
        ))}
      </TooltipProvider>
    </div>
  );
}
