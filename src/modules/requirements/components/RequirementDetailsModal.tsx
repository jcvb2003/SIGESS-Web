...
interface RequirementFormContentProps {
  requirement: NonNullable<ReturnType<typeof useRequirementDetail>["requirement"]>;
  events: ReturnType<typeof useRequirementDetail>["events"];
  onSave: (data: { status: RequirementStatus; numMte: string; recebido: boolean }) => Promise<void>;
  isSaving: boolean;
  onCancel: () => void;
}

function RequirementFormContent({ 
  requirement, 
  events, 
  onSave, 
  isSaving,
  onCancel 
}: RequirementFormContentProps) {
  const [status, setStatus] = useState<RequirementStatus>(requirement.status_mte);
  const [numMte, setNumMte] = useState(requirement.num_req_mte || "");
  const [recebido, setRecebido] = useState(requirement.beneficio_recebido);
  ...
  return (
    <>
      ...
      <DialogFooter className="px-6 py-4 border-t bg-muted/30">
        ...
        <Button 
          onClick={() => onSave({ status, numMte, recebido })} 
          disabled={isSaving}
        >
          {isSaving ? (
            <>
...
export function RequirementDetailsModal({
  ...
  const handleSave = async (data: { status: RequirementStatus; numMte: string; recebido: boolean }) => {
    if (!requirementId || !requirement) return;
    
    setIsSaving(true);
    try {
      await requirementService.updateStatus(requirementId, requirement.ano_referencia, data.status, {
        num_req_mte: data.numMte || null
      });

      if (data.recebido !== requirement.beneficio_recebido) {
        await requirementService.confirmBeneficio(requirementId, requirement.ano_referencia, data.recebido);
      }
    ...
    <Dialog open={!!requirementId} onOpenChange={(open) => !open && onOpenChange(false)}>
      <DialogContent className="max-w-2xl max-h-[90vh] flex flex-col p-0 overflow-hidden">
        ...
        {isLoading ? (
          ...
          <div className="flex-1 p-12 text-center text-muted-foreground">
            Erro ao carregar dados do requerimento.
          </div>
        )}
      </DialogContent>
    </Dialog>
