...
export default function FinancePage() {
  ...
  const statementMember = useMemo(
    () => members.find((m) => m.cpf === statementCpf),
    [members, statementCpf],
  );

  // Payment Dialog (Entity)
  const [paymentCpf, setPaymentCpf] = useState<string | null>(null);
  const paymentMember = useMemo(
    () => members.find((m) => m.cpf === paymentCpf),
    [members, paymentCpf],
  );

  // DAE Dialog (Repasse)
  const [daeCpf, setDaeCpf] = useState<string | null>(null);
  const daeMember = useMemo(
    () => members.find((m) => m.cpf === daeCpf),
    [members, daeCpf],
  );
  ...
  const stats = useMemo(() => {
    const inadimplentes = members.filter((m) => m.status === "overdue").length;
    const liberados = members.filter((m) => m.status === "released").length;
    const isentos = members.filter((m) => m.status === "exempt").length;
    return {
      arrecadadoMes: 0,
      qtdPagamentosMes: 0,
      inadimplentes,
      inadimplentes1Ano: Math.floor(inadimplentes * 0.6),
      daePendente: 0,
      liberados,
      isentos,
    };
  ...
  const tabCounts = useMemo(() => ({
    todos: total,
    "em-dia": members.filter((m) => m.status === "ok").length,
    inadimplentes: members.filter((m) => m.status === "overdue").length,
    liberados: members.filter((m) => m.status === "released").length,
    isentos: members.filter((m) => m.status === "exempt").length,
  }), [members, total]);
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-top-4 duration-500 pb-10">
      ...
      {/* Summary Cards */}
      <SummaryCards
        arrecadadoMes={stats.arrecadadoMes}
        qtdPagamentosMes={stats.qtdPagamentosMes}
        mesLabel={`${MONTH_NAMES[currentMonth]}/${String(currentYear).slice(-2)}`}
        inadimplentes={stats.inadimplentes}
        inadimplentes1Ano={stats.inadimplentes1Ano}
        daePendente={stats.daePendente}
        liberados={stats.liberados}
        isentos={stats.isentos}
      />
